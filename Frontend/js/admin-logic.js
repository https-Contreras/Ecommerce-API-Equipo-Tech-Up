document.addEventListener('DOMContentLoaded', () => {
    ...
    if (form) {
        form.addEventListener('submit', async (e) => {
            ...
            if (prodId) {
                method = 'PUT';
                url = `https://tech-up.westus2.cloudapp.azure.com/tech-up/api/admin/products/${prodId}`;
            } else {
                method = 'POST';
                url = 'https://tech-up.westus2.cloudapp.azure.com/tech-up/api/admin/products';
            }
...
async function cargarTablaProductos(categoria = 'all') {
    ...
    try {
        ...
        let url = 'https://tech-up.westus2.cloudapp.azure.com/tech-up/api/products';
        if (categoria !== 'all') {
...
window.eliminarProducto = async (id) => {
    ...
    if (result.isConfirmed) {
        try {
            ...
            const response = await fetch(`https://tech-up.westus2.cloudapp.azure.com/tech-up/api/admin/products/${id}`, {
                method: 'DELETE',
...
window.editarProducto = async (id) => {
    try {
        const response = await fetch(`https://tech-up.westus2.cloudapp.azure.com/tech-up/api/products/${id}`);
        const json = await response.json();
...
async function cargarEstadisticas() {
    ...
    try {
        const token = localStorage.getItem('userToken');
        const response = await fetch('https://tech-up.westus2.cloudapp.azure.com/tech-up/api/admin/dashboard-stats', {
            headers: { 'Authorization': `Bearer ${token}` }
        });
        const json = await response.json();
